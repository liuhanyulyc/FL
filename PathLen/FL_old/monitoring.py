...
def _install_monitoring_path(monitor, monitors, paths, links, adj_path):
    ...
    def install_SDN_path():
        ...
        for l2node in adj_path[2].keys():
            msg.actions.append(of.ofp_action_output(port=adj[monitor.dpid][l2node]))
        # print monitor.dpid
        temp_path = []
        temp_path.append(2)
        msg.actions.append(of.ofp_action_tp_port.set_src(tp_port=path_id[tuple(temp_path)]))
        ...
        for l2node in adj_path[2].keys():
            ...
            msg.match.nw_src = IPAddr('10.0.0.1')
            msg.match.nw_dst = IPAddr('10.0.0.2')
            msg.match.in_port = adj[l2node][monitor.dpid]
            
            #向下转发
            for l3node in adj_path[2][l2node].keys():
                msg.actions.append(of.ofp_action_nw_addr.set_src(nw_addr = switches_ip[l2node]))
        ...
        for l2node in adj_path[2].keys():
            for l3node in adj_path[2][l2node].keys():
                ...
                # 匹配
                msg.match.tp_dst, msg.match.tp_src = (None,) * 2
                msg.match.nw_src = switches_ip[l2node]
                msg.match.nw_dst = IPAddr('10.0.0.2')
                msg.match.in_port = adj[l3node][l2node]
                # 向下转发
                for l4node in adj_path[2][l2node][l3node]:
        ...
        for l2node in adj_path[2].keys():
            for l3node in adj_path[2][l2node].keys():
                for l4node in adj_path[2][l2node][l3node]:
                    ...
                    # 向下转发
                    for l5node in adj_path[2][l2node][l3node][l4node]:
                        msg.actions.append(of.ofp_action_output(port=adj[l4node][l5node]))
            ...
            for l3node in adj_path[2][l2node].keys():
                for l4node in adj_path[2][l2node][l3node]:
                    for l5node in adj_path[2][l2node][l3node][l4node]:
                        ...
                        msg.match.nw_src = switches_ip[l2node]
                        msg.match.nw_dst = IPAddr('10.0.0.2')
                        msg.match.in_port = adj[l5node][l4node]

                        #modify and send back
                        temp_path = [];temp_path.append(2);temp_path.append(l2node)
                        temp_path.append(l3node);temp_path.append(l4node)
                        temp_path.append(l5node);temp_path = tuple(temp_path)
                        msg.actions.append(of.ofp_action_tp_port.set_src(tp_port=path_id[temp_path]))
                        msg.actions.append(of.ofp_action_output(port=of.OFPP_IN_PORT))
                        switches[l5node].connection.send(msg)
            ...
            for l3node in adj_path[2][l2node].keys():
                for l4node in adj_path[2][l2node][l3node]:
                    for l5node in adj_path[2][l2node][l3node][l4node]:
                        ...
                        msg.match.nw_src = switches_ip[l2node]
                        msg.match.nw_dst = IPAddr('10.0.0.2')
                        msg.match.in_port = adj[l4node][l5node]
                        #往上转发
                        msg.actions.append(of.ofp_action_output(port=adj[l4node][l3node]))
                        switches[l4node].connection.send(msg)
        #l3收到l4的probe以后
        for l2node in adj_path[2].keys():
            for l3node in adj_path[2][l2node].keys():
        ...
        for l2node in adj_path[2].keys():
            for l3node in adj_path[2][l2node].keys():
                ...
                #往上转发
                msg.actions.append(of.ofp_action_output(port=adj[l2node][monitor.dpid]))
                switches[l2node].connection.send(msg)
        ...
        for l2node in adj_path[2].keys():
            ...
            msg.match.nw_dst = IPAddr('10.0.0.2')
            msg.match.nw_src = switches_ip[l2node]
            msg.match.in_port = adj[monitor.dpid][l2node]
