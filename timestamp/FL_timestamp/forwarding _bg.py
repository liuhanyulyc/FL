    ...
    def _handle_PacketIn(self, event):
       ...
       #通知ovs丢弃数据包
        ...
        def arp_k2_response(a):
            ...
            r.hwsrc = EthAddr('00:00:00:00:00:12')
            e = ethernet(type=packet.type, src=EthAddr('00:00:00:00:00:12'),
                        dst=a.hwsrc)
            e.payload = r
        ...
        def arp_k4_response(a):
            ...
            r.protodst = a.protosrc
            r.protosrc = a.protodst
            r.hwsrc = EthAddr('00:00:00:00:00:14')
            e = ethernet(type=packet.type, src=EthAddr('00:00:00:00:00:14'),
                        dst=a.hwsrc)
            e.payload = r
            if packet.type == ethernet.VLAN_TYPE:
                v_rcv = packet.find('vlan')
                    ...
                    if a.hwtype == arp.HW_TYPE_ETHERNET:
                        if a.protosrc != 0:
                            if a.opcode == arp.REQUEST:
                                # Maybe we can answer
                                if a.protodst == IPAddr('10.0.0.12'):
                                    arp_k2_response(a)
                                elif a.protodst == IPAddr('10.0.0.14'):
                                    arp_k4_response(a)
        #探测到10.0.0.1的udp报文raiseEvent NewMonitor事件，通知Monitoring下发流表
            ...
            if ip_packet.srcip != IPAddr('10.0.0.1') or ip_packet.dstip != IPAddr('10.0.0.2'):
