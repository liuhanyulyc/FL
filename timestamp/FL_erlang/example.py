...
class MyTopo(Topo):
    ...
    def __init__(self, n=2, **opts):
        ...
        for l in links:
            ...
            self.addLink(switches[l[0]], switches[l[1]], delay="%.2fms"%delay, bw=2000)
        ...
        Links = {}
        for p in paths:
            if not Links.has_key(p[-2]):
                if not p[-2] in monitors:
                    Links[p[-2]] = self.addLink(switches[p[-2]], switches['r'])
        #nodes to send and receive probe packet
# #-------------------------------------从这里开始，420----------------------------------------
